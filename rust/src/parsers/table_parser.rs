...
fn extract_course_list_from_raw_html(
    ...
    .map_err(|e| SiaScraperError::ParseError(format!("Failed to parse row regex: {}", e)))?;

    let span_regex = Regex::new(
        r#"(?is)<span[^>]*class\s*=\s*[\"'][^\"']*\baf_column_data-container\b[^\"']*[\"'][^>]*>(.*?)</span>"#,
    )
    .map_err(|e| SiaScraperError::ParseError(format!("Failed to parse span regex: {}", e)))?;
...
pub fn get_course_list(
    ...
    let row_selector = Selector::parse("tr.af_table_data-row")
        .map_err(|e| SiaScraperError::ParseError(format!("Failed to parse row selector: {}", e)))?;

    let span_selector = Selector::parse("span.af_column_data-container").map_err(|e| {
        SiaScraperError::ParseError(format!("Failed to parse span selector: {}", e))
    })?;
